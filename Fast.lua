...
return send(data.chat_id,data.id,"※ تم حفظ الكروب الدعم بنجاح \n※ جاري التشغيل","md",true)  
else
...
return send(data.chat_id,data.id,"※ الوضع المجاني معطل من قبل مطور الصانع @F_F_F_V")
end
if text == "※ الغاء" then
Redis:del(Fast..data.sender_id.user_id.."bottoken")
Redis:del(Fast..data.sender_id.user_id.."dev:user")
...
send(data.chat_id,data.id,"※ اهلا بك في صانع بوتات ميوزك سورس سمارك \n※ قناه السورس البوت @BV_YQ \n مبرمج السورس: @F_F_F_V ","html",true, false, false, true, reply_markup)
return false 
end
---making user
